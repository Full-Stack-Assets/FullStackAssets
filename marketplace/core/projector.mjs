...
export async function projectCanonEvent(repo, rawEvent, canonicalEntity) {
  ...
  return repo.transaction(async (tx) => {
    ...
    const productVersion = createProductVersion({
      id: deterministicVersionId(product.id, version, event.content_hash),
      product_id: product.id,
      version,
      canonical_snapshot: {
        refs: [event.entity_id],
        hashes: [event.content_hash],
        canon_versions: [version],
      },
    ...
    await tx.putProjectionReceipt({
      fingerprint, event_id:event.id, entity_type:event.entity_type, entity_id:event.entity_id,
      canonical_version:version, content_hash:event.content_hash, product_id:product.id,
      product_version_id:productVersion.id, result_state:isNewProduct ? 'CREATED' : 'VERSION_CREATED',
    });
    if (isNewProduct) {
      await tx.appendOutbox({
        id: outboxId('PRODUCT_CREATED', product.id, event.id),
        event_type:'PRODUCT_CREATED', aggregate_type:'PRODUCT', aggregate_id:product.id,
        payload:{ canonical_ref:event.entity_id },
      });
    }
    await tx.appendOutbox({
      id: outboxId('PRODUCT_VERSION_CREATED', productVersion.id, event.id),
      event_type:'PRODUCT_VERSION_CREATED', aggregate_type:'PRODUCT_VERSION', aggregate_id:productVersion.id,
      payload:{ product_id:product.id, canonical_ref:event.entity_id, version },
    });
