...
test('client preserves prior render on refresh failure and exposes retry',()=>{const js=readFileSync('assets/my-library.js','utf8');assert.match(js,/lastRendered/);assert.match(js,/lastSuccessAt/);assert.match(js,/data-library-retry/);assert.match(js,/credentials:'include'/);});
