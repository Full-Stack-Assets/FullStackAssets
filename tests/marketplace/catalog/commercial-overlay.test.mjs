...
test('commercial overlay promotes exactly the first ten without mutating Canon identity fields',async()=>{
  ...
  const cohortEntries=FIRST_COHORT.map((product,index)=>({id:product.canonical_ref,type:product.type,slug:index===0?'original':product.canonical_ref.toLowerCase(),name:index===0?'Canonical Name':product.name,description:index===0?'Canonical description':product.purpose,domain:'D',commercial_state:'REFERENCE_ONLY'}));
  const base={generated_at:null,publishers:[],taxonomy:{domains:['D'],types:['AGENT','SKILL'],commercial_states:['REFERENCE_ONLY']},entries:[...cohortEntries,{id:'SKL-999',type:'SKILL',slug:'other',name:'Other',description:'Other description',domain:'D',commercial_state:'REFERENCE_ONLY'}]};
  ...
  const promoted=result.entries.find(e=>e.id==='SKL-001');
  const untouched=result.entries.find(e=>e.id==='SKL-999');
  ...
  assert.equal(promoted.offer_summary.product_id,'PRD-0BD1D7BD75AE');
...
test('overlay refuses unknown Canon IDs and Canon identity changes',async()=>{
  assert.equal(existsSync(path),true,'commercial overlay module must exist');
  if(!existsSync(path))return;
  const {applyCommercialOverlay}=await import('../../../marketplace/catalog/commercial-overlay.mjs');
  const base={entries:[{id:'SKL-001',type:'SKILL',slug:'s',name:'N',commercial_state:'REFERENCE_ONLY'}],publishers:[],taxonomy:{}};
  assert.throws(()=>applyCommercialOverlay(base,{entries:{'SKL-999':{commercial_state:'FREE'}}}),/OVERLAY_UNKNOWN_PRODUCT/);
  assert.throws(()=>applyCommercialOverlay(base,{entries:{'SKL-001':{name:'Changed'}}}),/OVERLAY_FIELD_NOT_ALLOWED/);
});
